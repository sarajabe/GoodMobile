...
describe('Checkout - change shipping address and payment method from place order page', () => {
    before(() => {
    ...
    it('should assert the initial shipping address', () => {
        cy.get(':nth-child(1) > .card-tab > .tab-content > :nth-child(2)').should('have.text','home, 85 BROAD STNEW YORK, NY 10004-2434')
    });
    ...
    it('Should assert that the taxes fees are calculated correctly', () => {
        cy.get('[data-cy="taxesFees"]').should('have.text', '$0.27');
    });
    ...
    it('Should assert that the total equals to Total: $17.08.', () => {
        cy.get('.subtotal').should('have.text', ' Total: $17.08 ');
    });
    ...
    it('Should assert that the first address is already selected', () => {
        cy.get('[data-cy="shippingAddress"]').first().should('be.checked');
    });
    it('should select another shipping address successfully ', () => {
        PageObjects.ShippingPage.selectSecondShippingInfo();
    });
    it('should select shipping method', () => {
        cy.get('select').eq(1).select('Usps', { force: true }).should('have.value', 'usps');
        cy.get('select').eq(2).select('First Class Mail Shipping 3-7 Business days', { force: true }).should('have.value', 'usps_first_class_mail/letter');
    ...
    it('should assert that the shiping address was changed to', () => {
        cy.get(':nth-child(1) > .card-tab > .tab-content > :nth-child(2)').should('have.text','userg, 250 Vesey St New York, NY 10281');
    });
    it('should assert that the inital payment method', () => {
        cy.get('.card-tab > .sub-content').should('have.text','trial name, 12300 Bermuda Road24Henderson, NV 89044');
    });
    ...
    it('Should assert that the first payment was already selected', () => {
        cy.get('[data-cy="paymentMethod"]').first().should('be.checked');
    });
    it('should select another payment method', () => {
    ...
    it('should assert that the new payment method', () => {
        cy.get('.card-tab > .sub-content').should('have.text','Mariana Haddad, 388 Greenwich StNew York, NY 10013');
    });
    it('Should assert fees', () => {
        cy.get('[data-cy="subchargesFess"]').should('have.text','$1.63');
