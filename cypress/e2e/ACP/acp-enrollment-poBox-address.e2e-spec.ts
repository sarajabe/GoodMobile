...
describe('Sign in then enroll in ACP plan - new user - address shouldn`t be PO BOX', () => {
    before(() => {
    ...
    // fill in PO BOX address 
    it(`Should fill in address info`, () => {
        PageObjects.Acp.fillInPhysicalAddressInfo(CONSTANT.ACP_DATA.PO_BOX_ADDRESS.ADDRESS_LINE1,
            CONSTANT.ACP_DATA.PO_BOX_ADDRESS.CITY);
    });
    it(`Should fill in address info - the state and the zip`, () => {
        PageObjects.Acp.fillInPhysicalAddressInfo2(
            CONSTANT.ACP_DATA.PO_BOX_ADDRESS.STATE,
            CONSTANT.ACP_DATA.PO_BOX_ADDRESS.ZIP);
    });
    it(`Should fill in maling address info - the address line and the city`, () => {
        PageObjects.Acp.fillInMailingAddress(CONSTANT.ACP_DATA.PO_BOX_ADDRESS.ADDRESS_LINE1,
            CONSTANT.ACP_DATA.PO_BOX_ADDRESS.CITY);
    });
    it(`Should fill in maling address info - the state and the zip code`, () => {
        PageObjects.Acp.fillInMailingAddress2(CONSTANT.ACP_DATA.PO_BOX_ADDRESS.STATE,
            CONSTANT.ACP_DATA.PO_BOX_ADDRESS.ZIP);
    });
