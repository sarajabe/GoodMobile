...
describe('Sign in then enroll in ACP plan - new user - unrelated state and city', () => {
    ...
    it(`Should fill in personal info - birth  year`, () => {
    ...
    it('Should stay in acp page - and move to step two to choose the qualifying program', () => {
        PageObjects.TitleExpectations.goToACPEnrollemntPage();
    ...
    it(`Should fill in address & unrelated city`, () => {
        PageObjects.Acp.fillInPhysicalAddressInfo(CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .ADDRESS_LINE1,
            CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .CITY);
    });
    it(`Should fill in address info - unrelated state and the zip`, () => {
        PageObjects.Acp.fillInPhysicalAddressInfo2(
            CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .STATE,
            CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .ZIP);
    });
    it(`Should fill in maling address & unrelated city`, () => {
        PageObjects.Acp.fillInMailingAddress(CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .ADDRESS_LINE1,
            CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .CITY);
    });
    it(`Should fill in maling address info - unrelated state and the zip code`, () => {
        PageObjects.Acp.fillInMailingAddress2(CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .STATE,
            CONSTANT.ACP_DATA.UNRELATED_STATE_CITY_ADDRESS_INFO .ZIP);
    });
    ...
    it(`Should validate the pop up content message`, () => {
        cy.get('.message').should('have.text','The Contact Address, City, State, and Zip Code are incomplete. Please provide a valid contact address');
    });
