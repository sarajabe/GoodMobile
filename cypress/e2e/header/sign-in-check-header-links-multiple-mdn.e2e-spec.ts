...
describe('Sign in to insure from the links in header menu - multiple mdn', () => {
    before(() => {
    ...
    it('Should assert the selected mobile number', () => {
        cy.get('select').find('option').eq(0).should('have.value','0: Object').should('contain',' (646) 201-3954 ');
    });
    it('Should assert the selected mobile number has changed in the account summary', () => {
        cy.get('select').eq(0).find('option').should('have.value','0: Object').should('contain',' (646) 201-3954 ');
    });
