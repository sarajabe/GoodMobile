...
describe('Purchase a phone - logged in user - chose att mdn - enter tmo zip - accept limitations and change to TMO', () => {
    before(() => {
    ...
    it('Should select a tmo mdn from the dropdown list ', () => {
        cy.get('select').eq(1).select('(586) 252-7260', { force: true }).should('have.value', '4: Object');
    });
    ...
    it('Should click on yes change to the preferred network', () => {
        PageObjects.PurchasedPhones.clickOnChangeThePreferredNetwork();
    });
