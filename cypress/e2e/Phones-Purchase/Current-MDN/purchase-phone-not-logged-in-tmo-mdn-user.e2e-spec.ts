...
describe('Purchase a phone - not logged in user - tmo mdn', () => {
     before(() => {
     ...
     it('Should click on sign in btn', () => {
          PageObjects.PurchasedPhones.clickOnSignInBtn();
     });
     ...
     it('Should select a tmo mdn from the dropdown list ', () => {
          cy.get('select').eq(0).select('(214) 517-1499', { force: true }).should('have.value', '0: Object');
     });
