...
@Component({
  selector: 'app-app-id-validation',
  templateUrl: './app-id-validation.component.html',
  styleUrls: ['./app-id-validation.component.scss']
})
export class AppIdValidationComponent implements OnInit {
  ...
  private checkVerifyStatus(result: any): void {
    ...
    if (!!result && result?.status) {
      this.enrolled = true;
      if (!!this.acpFinishAppUrl || result.status === "PENDING_REVIEW" || result.status === "IN_PROGRESS" || result?.status === 'PENDING_ELIGIBILITY') {
        this.acpError = true;
  ...
  private callVerifyAcp(renewRecaptcha?: boolean): void {
    ...
    let appData: IAppDetails = {
      customerId: this.customerId,
      applicationId: this.applicationId,
      command: 'CreateACPWithProviderAppIdCommand'
    };
