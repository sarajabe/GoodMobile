...
fdescribe('Yes Flow - EBB Personal Information Component - Unit Testing', async () => {
  beforeEach(async () => {
    ...
    appIdField = component.personalInfoForm.controls.applicationId;
  ...
  it('Should show a validation messages for the required elements when the values are empty without showing the invalid messages', waitForAsync(() => {
    fixture.whenStable().then(() => {
      ...
      const requiredAppIdMsg = fixture.debugElement.query(By.css('#required-applicationId-msg'));
      ...
      expect(appIdField.errors.required).toBeTruthy();
      expect(appIdField.errors.pattern).toBeFalsy();
      expect(requiredAppIdMsg.nativeElement).toBeDefined();
  ...
  it('Should show a validation messages for the required elements when the values are empty, then enter an invalid values to check that invalid messages appears, after that entering a valid values and check that all errors are null', waitForAsync(() => {
    fixture.whenStable().then(() => {
      ...
      expect(appIdField.errors.required).toBeTruthy();
      expect(appIdField.hasError('pattern')).toBeFalsy();
      expect(requiredAppIdMsg.nativeElement).toBeDefined();
      ...
      appIdField.setValue(ACP_MOCKS.INVALID_FULL_USER_INFO.appId);
      ...
      expect(appIdField.errors.required).toBeFalsy();
      ...
      appIdField.setValue(ACP_MOCKS.APPLICATION_ID.ebbId);
      appIdField.markAsTouched();
      appIdField.markAsDirty();
      fixture.detectChanges();
      ...
      expect(appIdField.errors).toBeFalsy();
  ...
  it('Should show a validation messages for the date is required, if we select the day as 30 or 31 and pick a 2 as a month as in check month function we set the day empty if there is an invalid date', waitForAsync(() => {
    fixture.whenStable().then(() => {
      component.savedInfo = { user: ACP_MOCKS.SAVED_INFO, appId: ACP_MOCKS.APPLICATION_ID.ebbId };
      fixture.detectChanges();
  ...
  it('Should populate the form when there is saved info already', waitForAsync(() => {
    fixture.whenStable().then(() => {
      ...
      expect(component.personalInfoForm.errors).toBeNull();
      expect(appIdField.value).toEqual(ACP_MOCKS.APPLICATION_ID.ebbId);
